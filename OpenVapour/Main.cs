...
namespace OpenVapour {
    public partial class Main : Form {
        ...
        private static List<Image> states = new();
        ...
        private void Main_Load(object sender, EventArgs e) {
            ...
            Bitmap background = new(Width, Height);
            LinearGradientBrush gradientbrush = new(new PointF(0, 0), new PointF(0, Height), Color.FromArgb(250, 149, 255), Color.FromArgb(173, 101, 255));
        ...
        internal Panel CreatePopUp(PictureBox selector) {
            ...
            Panel popup = new() { Size = new Size(320, 170), BackColor = Color.FromArgb(165, 0, 0, 0), ForeColor = Color.White, Visible = false };
            PictureBox gameart = new() { Location = new Point(5, 5), Size = new Size(107, 160), SizeMode = PictureBoxSizeMode.StretchImage, Image = pbi[0] };
            Label gamename = new() { AutoSize = true, Location = new Point(114, 5), MaximumSize = new Size(201, 35), Font = new Font("Segoe UI Light", 18f, FontStyle.Regular), Text = game.Name, BackColor = Color.Transparent };
            Label gameabout = new() { AutoSize = true, Location = new Point(117, 43), MaximumSize = new Size(198, 117), Font = new Font("Segoe UI Light", 12f, FontStyle.Regular), Text = game.GetStrippedValue("detailed_description"), BackColor = Color.Transparent };
        ...
        internal Panel CreatePopUp(ResultTorrent torrent, Image gameimage) {
            Panel popup = new() { Size = new Size(320, 170), BackColor = Color.FromArgb(165, 0, 0, 0), ForeColor = Color.White, Visible = false };
            PictureBox gameart = new() { Location = new Point(5, 5), Size = new Size(107, 160), SizeMode = PictureBoxSizeMode.StretchImage, Image = gameimage };
            Label gamename = new() { AutoSize = true, Location = new Point(114, 5), MaximumSize = new Size(201, 35), Font = new Font("Segoe UI Light", 12f, FontStyle.Regular), Text = torrent.Name, BackColor = Color.Transparent };
            Label gameabout = new() { AutoSize = true, Location = new Point(117, 43), MaximumSize = new Size(198, 117), Font = new Font("Segoe UI Light", 12f, FontStyle.Regular), Text = torrent.Description, BackColor = Color.Transparent };
            gamename.Font = Utilities.FitFont(gamename.Font, gamename.Text, gamename.MaximumSize);
        ...
        internal void AddTorrent(ResultTorrent torrent) {
            PictureBox panel = new() { Size = new Size(225, 225), SizeMode = PictureBoxSizeMode.StretchImage, Margin = new Padding(5, 7, 5, 7), Cursor = Cursors.Hand }; //panel.Paint += Utilities.dropShadow;
            ...
            try {
                ...
                List<object> metalist = new() { states, torrent };
        ...
        internal async void AddGame(SteamGame game) {
            PictureBox panel = new() { Size = new Size(150, 225), SizeMode = PictureBoxSizeMode.StretchImage, Margin = new Padding(5, 7, 5, 7), Cursor = Cursors.Hand }; //panel.Paint += Utilities.dropShadow;

            List<object> metalist = new() { states, game };
            panel.Image = states[0];
            panel.Tag = metalist;
        ...
        private void GameClick(object sender, EventArgs e) {
            ...
            if (!gamepanelopen) { LoadGame(game, pbs[0]); } else if (panelgame != game.Name) { ClosePanel(true, pbl); }}
        ...
        private SteamGame currentgame = new("");
        ...
        private ResultTorrent currenttorrent = new("");
        ...
        private void ClosePanel(bool OpenNext, List<object> List) {
            ...
            time.Tick += delegate {
                ...
                else if (OpenNext) { LoadGame(game, pbs[0]); panelgame = ""; time.Enabled = false; } else { panelgame = ""; time.Enabled = false; }};
            time.Enabled = true; }
        ...
        private void ClosePanelTorrent(bool OpenNext, List<object> List) {
            ...
            time.Tick += delegate {
                if (gamepanel.Location.X > -gamepanel.Width) gamepanel.Location = new Point(gamepanel.Location.X - 90, gamepanel.Location.Y);
                else if (OpenNext) { LoadTorrent(game, pbs[0]); panelgame = ""; time.Enabled = false; } else { panelgame = ""; time.Enabled = false; }
            }; time.Enabled = true; }
        ...
        private void DrawSearchBox(object sender, EventArgs e) {
            Bitmap bit = new(searchtextbox.Width, searchtextbox.Height);
            ...
            if (e.KeyCode == Keys.Enter) {
                ...
                foreach (ResultGame game in await SteamCore.GetResults(realsearchtb.Text)) {
                    if (!Cache.IsSteamGameCached(game.AppId)) {
                        if (!await Utilities.IsDlc(game.AppId.ToString())) { AsyncAddGame(game.AppId); await Task.Delay(50); }
                    } else { /*AddGame(game);*/ }}}}
        ...
        private async void TorrentSearch(object sender, EventArgs e) {
            ...
            foreach (ResultTorrent torrent in await GetResults(AlphanumericRegex().Replace(currentgame.Name, string.Empty).Replace("  ", " ").Replace("  ", " "))) AddTorrent(torrent); }
        ...
        private async void Magnet(object sender, EventArgs e) {
            ...
            try {
                if (magnet.Length > 0) {
                    ...
                    Process.Start(magnet); // Process.Start will (usually) throw an exception if no magnet-capable applications are installed
        ...
        private async void MainShown(object sender, EventArgs e) {
            store.Visible = true; toolbar.Visible = true; if (Directory.GetFiles(Utilities.RoamingAppData + "\\lily.software\\OpenVapour\\Storage\\Games").Length > 0)
                foreach (string file in Directory.GetFiles(Utilities.RoamingAppData + "\\lily.software\\OpenVapour\\Storage\\Games")) {
                    try { AddGame(await GetGame(Convert.ToInt32(file[(file.LastIndexOf("\\") + 1)..]))); } catch (Exception ex) { Utilities.HandleException($"MainShown(sender, e)", ex); }}
            else { store.Controls.Add(nogamesnotif); nogamesnotif.Visible = true; }
            Timer textboxcursor = new() { Interval = 128 };
